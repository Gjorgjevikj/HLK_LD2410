...
void setup()
{
    ...
    Serial.println("TRYING to... Read firmware version");
    ...
    if(d == HLK_LD2410::DR_ERROR)
        Serial.println("Reading distance resolution failed!");
    else
    ...
    */
    ...
    delay(500);
    Serial.println("Setting distance resolution to 0.75m");
    if (radar.setDistanceResolution(HLK_LD2410::DR_0_75))
        Serial.println("Distance resolution set to 0.75");
    else
        Serial.println("Setting distance resolution failed");
    delay(200);
...
//    Serial.printf("DistanceResolution now : %04x (%s)\n", d2, (d2 < 2 ? dr[d2] : dr[d2]));
    ...
    Serial.println("Setting DistanceAndDuration...");
    if (radar.setDistanceAndDuration(8, 8, 2))
        Serial.println("setDistanceAndDuration(...) succsess");
    else
        Serial.println("setDistanceAndDuration(...) failed");
    ...
    Serial.println("Disabling Engineering mode...");
    if (radar.disableEngineeringMode())
        Serial.println("Disabling EngineeringMode sucess");
    else
        Serial.println("Disabling EngineeringMode failed");
    delay(200);

/*
    Serial.println("TRYING to...    Reading firmware version" );
    HLK_LD2410::FirmwareVersion ver = radar.reqFirmwareVersion();
    Serial.printf("FirmwareVersion %x.%02x.%08x (type%x)\n", ver.major, ver.minor, ver.bugfix, ver.type);
    delay(200);
    
    Serial.println("TRYING to...    setBaudRate");
    if (!radar.setBaudRate(HLK_LD2410::BAUD_256000))
        Serial.println("setBaudRate failed.");
    else
        Serial.println("setBaudRate sucess.");
    */
...
void loop()
{
    if (!emode && millis() - loopStart > 5000)
    {
        emode = true;
